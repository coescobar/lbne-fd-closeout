...
the number of serial links for data tranmission will be cut in half.
...
The ADC ASIC has 12-bit resolution, 2~MS/s sampling rate, built in FIFO, two 8:1 mulplexing and two pairs of serialized output.
...
The data from the 16 ADCs is transferred parallelly to the FIFO block.
...
The ADC along with the input buffers are biased internally using a bias generator and a bangap voltage reference.
The bangap voltage (VBGR) can be monitored and/or controlled externally.
...
The differential non-liearity (DNL) is less than 4 LSBs for 99\% of ADC bins at both 300~K and 77~K.
...
A total of 8,256 channels (516 FE ASICs) have been used to intrument the MicroBooNE TPC.
...
The development of the FE and ADC ASICs has proceded through a series of prototype designs.
...
using a commerical FPGA evaluation board as a mini-DAQ system.
...
This inlcudes key parameters (gain, noise, non-linarity etc.) that should be tested,
...
It is an important step to identify potential issue, check out system intergration and performance
