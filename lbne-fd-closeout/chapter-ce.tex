...
\fixme{Need an intro sentence or short pgraph; decide if we need to define terms, e.g., ASIC, CMOS...
\newline rwh: I added definitions of ASIC and CMOS, but, frankly, ``everyone'' in the field knows what they are,
and this has the appearance of defining water.}

\fixme{This chapter needs a good illustration showing the placement of the electonics with respect to the APAs and the cryostat, feedthroughs, etc.}
...
\fixme{This is a response to the requirement; should be elsewhere
\newline rwh: This was part of my thought process.  I didn't mean to leave it here.  Now it's moved.}
...
\end{cdrfigure}
\fixme{Check white space around figure
\newline rwh: Done, but there wasn't much white space.  I assume this refers to Fig.~\ref{fig:elect_schem}.}
...
\end{cdrfigure}
\fixme{Should these images be annotated, or are they meaningful as is to the intended audience?
\newline rwh: I don't understand what you are asking here.}
...
\item Four 1-Gbps serial drivers that form the data link to DAQ \fixme{I don't understand the relationship between driver and link
\newline rwh: These are electronic terms, and I think the meaning is clear.}
\end{itemize}
...
\fixme{This could use a bit more of an intro; we get to ``thus and MOS transistor has blah...'' and I don't even know what MOS is (well, I've certainly heard of it, but in terms of the text) or how it relates to the issues brought up}
...
The goal of this part of the program \fixme{have you described a program?} is to use accelerated testing to verify the expected lifetimes for the several
...
%. These methods are (you went from `a method' to `these methods')
...
\end{cdrfigure}
\fixme{V sub DS not defined
\newline rwh: This is a very standard notation.  Anyone familiar with MOS etc. will understand this.
Anyone not familiar with MOS etc. is not likely to get anything out of this section whether this variable is defined or not.}
...
A monitor of the impact ionization is the bulk current, which reaches a maximum at $V_{DS} = V_{DD}$ and at $V_{GS} = 0.5 V_{DD}$. \fixme{V sub * not defined
\newline rwh:  See previous response to V sub DS.}
...
compensating for the increased L. \fixme{not a sentence.
\newline rwh: Now it is.}
%
...
digitally programmable gain and peaking time as described above. \fixme{which section?}
Each channel also implements a high-performance output driver,
which is used to drive a long cable when it is used in a standalone mode, as it is in MicroBooNE. \fixme{need reference?}
...
\fixme{This is sort of a repeat of lines 273ff; please fix.
\newline rwh: I can't find any line labeled 273.  This goes to my earlier request about doing the line numbers differently.} The ADC ASIC has now been through four design/fabrication/testing revision cycles.
Prototypes from each cycle have been evaluated and characterized at room (300~K) and liquid nitrogen (77~K) temperatures.
During these tests the circuits have been cycled multiple times.
...
An analog front-end ASIC was adopted by the MicroBooNE experiment in 2010. \fixme{ref?}
...
%Both FE ASICs and ADC ASICs are being used in the 35-ton LArTPC prototype. \fixme{ref?}
...
automated place-and-route \fixme{automated placement and routing?
\newline rwh: I already had this conversation with the author of this phrase.  ``place and route'' is an industry-standard phrase, it turns out, so I decided to keep it, awkward though it may appear to us outside the industry.
I did add hyphens, which make it less so.} for design blocks using extensive digital logic.  \fixme{This sentence is a mouthful -- not sure how the last phrase fits in (for design...)
\newline rwh: Same comment. I think the hyphens help.}
...
the control/download information transmitted from the DAQ. \fixme{components are a downlink (required for xzy) and what else?
\newline rwh: I don't know what you mean here.} 
...
which will generate a low-jitter stable clock to the high-speed serializer. \fixme{it will generate a clock?}
...
All data will be transmitted off-board to DAQ. \fixme{to where?
\newline rwh: I thought it was obvious, so I took out ``to DAQ'', which I therefore regarded as dead words, but I put that back.}
...
\fixme{This last sentence is opaque to me; I think it's my ignorance of driver/link, but maybe you can improve it anyway 
\newline rwh: Sorry, I can't think of how to improve this without maybe adding a short lesson on the subject, which I'm not really qualified to do, and besides I think that would exceed the scope of the CDR.}
...
\fixme{You've given a reason that they CAN be copper, not why they SHOULD be copper.
\newline rwh: ``Copper'' means electrical, and in this sense serves to distinguish from optical. These are the only two choices. I added a phrase to that effect.}
...
Optical fiber will be employed externally to the cryostat \fixme{for the LVDS connections? 
\newline rwh: LVDS is defined above.  Nothing to do with optical. Again, I'm not sure what you're asking here.}, under DAQ scope. \fixme{Here's what I understand: slower connections will use optical fiber, data will use copper. Is that right?
\newline  rwh: No. Optical is far faster than copper, but optical coverters haven't been shown to work in the cold,
so there we are strictly using electrical (copper) connections. I added some text to that effect, just above here.}
...
running in the Atlas experiment for about 15 years, \fixme{ref?} demonstrating both longevity and reliability.
...
two or three \fixme{ok?
\newline rwh: Huh?} thousand volts with less than a couple of milliamps.
...
\fixme{This intro didn't flow... please check}

The power-per-channel for the FE ASIC is designed to be about 15~mW and
the total high-voltage \fixme{?
\newline rwh: What's wrong here? I don't see it.} power requirement for each APA is expected to be about 65~W. 

\fixme{The first kind:
\newline rwh: No, this is not the first kind.  The two ``kinds'' are not described, and this is not important.  I took out
the ``two kinds'' sentence, which was part of something else that was not completely removed, and which was misleading.} 
...
\fixme{where do we describe the second?
\newline rwh: See previous comment.}
%
%%%%%%%%%%%%%%%%
...
\fixme{Isn't this still the first kind?
\newline rwh: See previous comment.}
