...
With the uncertainty in the liquid argon HV dielectric strength, attention will be focused on the HV-related aspects of the TPC design; this appears to involve the CPAs and field cage modules. \fixme{right?} An R\&D document has been compiled for the far detector~\cite{fd_rndplan_10006} that contains a section on the proposed R\&D topics and activities to reduce the HV-related risks. Apart from the HV feedthrough, which will be designed and tested above the operating voltages, there is a plan to improve the designs of the CPAs and field cage modules.
...
The physics sensitivity of LBNE \fixme{it said DUNE, I think we want lbne} has been estimated based on detector-performance characteristics published in the literature, simulation-based estimates
...
\begin{cdrfigure}[ArgoNeut: tracks as seen on (deconvoluted) induction plane]{Argo-decon}{Figure from the ArgoNeuT draft NIM paper}
...
The MicroBooNE experiment is an 89-ton active mass LArTPC, (170-ton total \fixme{`total' is right?} argon mass) in the commissioning phase.  It has both a physics program and LArTPC development goals.  
