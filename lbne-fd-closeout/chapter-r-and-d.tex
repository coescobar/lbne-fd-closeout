...
\begin{cdrfigure}[Layout of 35t prototype at Fermilab's PC-4 facility]{35cryo}{Layout of 35t prototype at Fermilab's PC-4 facility. }
...
inertia of the LAr. However \fixme{need `however'?} the normalized drift time (= drift time/(average drift time for this period) is 
...
agreement with the current CFD calculations [5]\fixme{add real reference}.
...
The physics sensitivity of DUNE \fixme{this is the LBNE closeout; do we want to mention DUNE?} has been estimated based on detector-performance characteristics published in the literature, simulation-based estimates
...
\end{cdrfigure} %     STILL TO DO      
\fixme{Can you remove the caption from the PDF and add it to this caption?}
...
The MicroBooNE experiment is an 89-ton active mass LArTPC, (170-ton total \fixme{check} argon mass) in the commissioning phase.  It has both a physics program and LArTPC development goals.  
