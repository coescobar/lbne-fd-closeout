...
%\fixme{could we list out the components here? - I prefer not, I wanted to get the
%requirements in ahead of lists of names of things}
...
ICARUS. 
%\fixme{Need references? - If we had more time, ideally yes!} 
Guidance and experience is also available from the design and
operation of the LBNE 35t prototype 
%\fixme{internal reference? - No, we don't have anything to reference yet} 
...
%\fixme{what does it mean to ``contribute to a requirement?' - I mean that the rate is much higher from other sources and they drive the requirement, so this only adds a small increment to the bandidth we are required to provide.'}
...
With 
%\fixme{Erik suggests a simple diagram to accompany this paragraph - let's not - no time now} 
...
%\fixme{``data front-end computers''? Why `data' - that is what is in the figure where we separate the trigger-frontends from the data-fontends}
...
%For simplicity, this \fixme{the protocol?} maintains a 64\,MHz clock, but not a large counter
...
%\fixme{should or must? - should, see next paragraph} 
...
connected to the time-synchronization network as described in Section~\ref{sec:daq_time}. %\fixme{the prev section? Please use Section~\ref{sec:dyn-sync-exec} or whatever the right label is}
...
% as described above.\fixme{pls indicate section - ahem, there isn't any description above!}
...
\fixme{This section may still need some more cleanup}
...
%\fixme{You don't give any totals here; might be useful -- deliberately omitted, we don't know them yet!}
...
%\fixme{This sentence was a mouthful... does the previous sentence capture the right idea? - yes, excellent!!!}
...
% Thankyou, yes, this was a mess - I have reworded it
% This places a number of requirements on the readout
...
%It is possible that an alternative way for a \COMPARTMENT\
%to obtain the spill-timing information independently of the central
%manager may be available. \fixme{this sentence is pretty vague - yes you are right, let's leave it out}

The 
%\fixme{something about `modular'? - good idea} 
