...
\fixme{``As shown in Figure~\ref{fig:tpc-wire-frame-xsect}, the readout electronics reside only along one narrow edge of an APA.'' How is that shown? I assume it's at the top, but that's not said.} During installation, two APAs are interconnected on their non-readout ends, leaving the readout ends completely outside of the TPC's active volume.  Cables from the bottom APAs can be routed either through the hollow vertical APA frames, or down along the floor and up along a side wall of the cryostat.
...
at $\sim\pm$35.71$^\circ$ to the vertical. \fixme{why $\sim$ here?} This wire layout is shown to be very good for reconstructing beam-neutrino events \cite{wire-orientation}. The wires on the grid plane are not 
...
to minimize the surface electric field.  Two sets of field-shaping end pieces are installed at the two ends of the CPA wall to properly terminate the cathode wall with rounded edges.  All CPAs are suspended from the ceiling using G10 hangers to \fixme{electrically?} insulate the CPA from the cryostat.
...
Each facing pair of cathode and anode \fixme{``CPA and APA rows'' better?} rows forms an electron-drift region. A field cage  completely surrounds the four open sides of this region
...
and the CPA rows will be made through stainless-steel pipes in the 
